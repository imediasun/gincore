...
class Model
{
    ...
    public function get_goods_filters($url, $sort=null, $like = null )
    {
        if ( $url == 'all')
            $this_sql = '';
        elseif ( is_null($like) )
            $this_sql = $this->db->makeQuery('{category_goods}.category_id=(SELECT id FROM {categories} WHERE url=? AND avail=1) AND', array($url));
        else
            $this_sql = $this->db->makeQuery('{category_goods}.category_id IN (SELECT id FROM {categories} WHERE parent_id=(SELECT id FROM {categories} WHERE url=? AND avail=1)) AND', array($url));
        ...
        $sql=$this->db->makeQuery(
            ...
            WHERE ?query
            {goods_images}.goods_id={goods}.id AND {goods_images}.type=1' . $like . $sort[0] . ' , {goods_images}.prio DESC' , $array
        );
        ...
        foreach($sort_values as $key=>$value){
            ...
            if($active){
                ...
                $sort_list .= '<li'.($active ? ' class="active"' : '').'>'.
                    '<a href="'.$prefix.implode('/', $arrequest).'?sort='. $key . $field . '">'.$value.'</a>'.
                    '</li>';
            }
        }
    ...
    public function get_category($cat_id, $prefix, $cat_page, $crumbs_arrow, $begin=null, $product=null)
    {
        ...
        $category = $this->db->query('SELECT id,url,parent_id,title FROM {categories} WHERE id=?i',array($cat_id))->row();

        if ( $category['parent_id'] > 0 ) {
            $str = $this->get_category($category['parent_id'], $prefix, $cat_page, $crumbs_arrow, $begin, $product) . $crumbs_arrow;
        }
    ...
    function get_prices($price, $course_key = false, $course_value = 0) {
        ...
        if ( $course_key && $course_value > 0 ) $settings[$course_key] = $course_value;
...
/*
    function get_current_price($price){
        global $cfg;
        $user_currency = $_COOKIE[$cfg['tbl'].'currency'];
        ...
        switch($user_currency){
            case 'currency-dollar':
                ...
                break;
            case 'currency-grn':
                return $prices['grn-cash'];
                break;
            case 'currency-grn-vat':
                return $prices['grn-vat'];
                break;
            case 'currency-grn-noncash':
                return $prices['grn-noncash'];
                break;
            default:
                return $price;
                break;
