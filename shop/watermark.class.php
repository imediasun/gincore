...
class Watermark {
            ...
            if ($image){
                switch ($this->type){
                    ...
                    case Watermark::BOTTOM_RIGHT_SMALL:
                        ...
                        break;
                    case Watermark::BOTTOM_CENTER:
                        return $this->watermark_bottom_center($image, $watermark);
                        break;
                }
                return true;
            }else {
    ...
    private function watermark_bottom_center (&$image, &$watermark)
    {
        $size = getimagesize($this->image);
        $im_x = $size[0];
        $im_y = $size[1];
        ...
        ImageCopyResampled ($watermark_mini, $watermark, 0, 0, 0, 0, $w, $h, $w, $h);
        ...
        if (!function_exists($functions[$this->img_type])) {
            return false;
        }
        $functions[$this->img_type]($image, $this->output_file);

        /*imagejpeg($image,$this->output_file, 100);
        imagepng($image,$this->output_file);
        imagegif($image,$this->output_file);
        imagewbmp($image,$this->output_file);*/
