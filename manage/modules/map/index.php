...
class map
{
    ...
    function __construct($all_configs, $lang, $def_lang, $langs)
    {
        ...
        if (!$all_configs['oRole']->hasPrivilege('edit-map')) {
            return false;
        }
    ...
    private function gen_menu_map_level($parent, $section, $def_lang)
    {
        ...
        $sql = $this->all_configs['db']->query("SELECT id, page_type, state, url, parent, 
                           ...
                           FROM {map} as m WHERE parent=?i AND section=?i ORDER BY prio", array($parent, $section),
            'assoc');
        ...
        foreach ($sql as $pp) {
            ...
            $sql1 = $this->all_configs['db']->query("SELECT id FROM {map} WHERE parent=?i ORDER BY prio",
                array($pp['id']), 'ar');
     ...
     * @global string $path
     * @global array  $config
     * @internal param array $mage_modules
     */
    ...
    private function save_modules_from_post($page_id)
    {
        ...
        if (isset($_POST['module']) && $_POST['module']) {
            ...
            $this->all_configs['db']->query("INSERT INTO {map_module} (page_id, module) VALUES ?q",
    ...
    protected function out_choose_picture($pp)
    {
        ...
        $parent_page_type = $this->all_configs['db']->query("SELECT page_type FROM {map} WHERE id = ?i",
            array($pp['parent']), 'el');
    ...
    {
        $sql0 = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent=0 AND section=?i ORDER BY prio",
            array(1), 'assoc:id');
        $parentoption = '';

        $translates0 = get_few_translates(
            'map', 'map_id',
            $this->all_configs['db']->makeQuery("map_id IN(?q)", array(implode(',', array_keys($sql0))))
        );
        foreach ($sql0 as $pp0) {
            ...
            $sql1 = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent = ?i AND section = ?i ORDER BY section, prio",
                array($pp0['id'], 1), 'assoc:id');
            if ($sql1) {
                $translates1 = get_few_translates(
                    'map', 'map_id',
                    $this->all_configs['db']->makeQuery("map_id IN(?q)", array(implode(',', array_keys($sql1))))
                );
                foreach ($sql1 as $pp1) {
                    ...
                    $sql2 = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent = ?i AND section = ?i ORDER BY section, prio",
                        array($pp1['id'], 1), 'assoc:id');
                    if ($sql2) {
                        $optdis2 = '';
                        $translates2 = get_few_translates(
                            'map', 'map_id',
                            $this->all_configs['db']->makeQuery("map_id IN(?q)", array(implode(',', array_keys($sql2))))
                        );
                        foreach ($sql2 as $pp2) {
                            $pp2t = translates_for_page($this->lang, $this->def_lang, $translates2[$pp2['id']], $pp0,
                                true);
                            $optsel2 = $page_parent == $pp2['id'] ? 'selected' : '';
                            $parentoption .= '<option ' . $optdis2 . $optsel2 . ' value="' . $pp2['id'] . '">- - = ' . $pp2t['name'] . '</option>';

                            $sql3 = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent = ?i AND section = ?i ORDER BY section, prio",
                                array($pp2['id'], 1), 'assoc:id');
                            if ($sql3) {
                                $translates3 = get_few_translates(
                                    'map', 'map_id', $this->all_configs['db']->makeQuery("map_id IN(?q)",
                                    array(implode(',', array_keys($sql3))))
                                );
                                foreach ($sql3 as $pp3) {
                                    $pp3t = translates_for_page($this->lang, $this->def_lang, $translates3[$pp3['id']],
                                        $pp3, true);
                                    $optsel3 = $page_parent == $pp3['id'] ? 'selected' : '';
                                    $parentoption .= '<option ' . $optsel3 . ' value="' . $pp3['id'] . '">- - = = ' . $pp3t['name'] . '</option>';

                                    $sql4 = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent = ?i AND section = ?i ORDER BY section, prio",
                                        array($pp3['id'], 1), 'assoc:id');
                                    if ($sql4) {
                                        $translates4 = get_few_translates(
                                            'map', 'map_id', $this->all_configs['db']->makeQuery("map_id IN(?q)",
                                            array(implode(',', array_keys($sql4))))
                                        );
                                        foreach ($sql4 as $pp4) {
                                            $pp4t = translates_for_page($this->lang, $this->def_lang,
                                                $translates4[$pp4['id']], $pp4, true);
        ...
        if ($query && $fields) {
            if ($query && $fields) {
                ...
                foreach ($fields as $field => $v) {
                    $where[] = $this->all_configs['db']->makeQuery(" `" . mysql_real_escape_string($field) . "` LIKE ? ",
                        array('%' . $query . '%'));
                }
...
###############################################################################
        if (isset($this->all_configs['arrequest'][1]) && is_numeric($this->all_configs['arrequest'][1])) {
            $pp = $this->all_configs['db']->query("SELECT * FROM {map} WHERE id=?i",
            ...
            $sqlmod = $this->all_configs['db']->query("SELECT * FROM {map_module} WHERE page_id = ?i", array($pp['id']),
                'assoc');
            $page_modules = array();
            ...
            if (!isset($this->all_configs['arrequest'][2]) || substr($this->all_configs['arrequest'][2], 0,
                    3) == 'mod'
            ) {
                ...
                $out .= '      
                    ...
                    ($pp['buy_old'] == 1 ? 'checked="checked"' : '') . '> ' . l('map_buy_old') . '</label>
                            ...
                            ' . l('map_hotline_price') . ' ' . $pp['hotline_price'] . ' ' . viewCurrency() . '
                        </div>
                        <div class="form-group">
                ...
                $sql0 = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent = ?i AND section = ?i ORDER BY section, prio",
                    array($pp['id'], $pp['section']), 'assoc:id');
                ...
                $out .= '<div class="form-group"><label>' . l('map_page_date') . '</label>'
                    . '<input class="form-control" type="text" name="uxt" value="' . date("Y-m-d H:i:s",
                        strtotime($pp['uxt'])) . '" size="10" id="datetimepick" /></div> ';
                $out .= '<br><fieldset>
                            ...
                            <div class="form-group">
                                ...
                                ' . $this->gen_templates_list('body_header', 'template_body_header',
                        $pp['template_body_header']) . '
                            </div>
                            <div class="form-group">
...
<!-- Цены -->
                        <div class="tab-pane" id="prices">
                            ...
                            <h4>' . l('Услуги') . '</h4>
                                ' . Inc_helper_map_price::show_tables_by_type($pp['id'], 1,
                        $this->lang == $this->def_lang) . '
                            <h4>' . l('Ремонт') . '</h4>
                                ' . Inc_helper_map_price::show_tables_by_type($pp['id'], 2,
                        $this->lang == $this->def_lang) . '
                       </div>
...
<!-- сортировка -->
                       <!--<div class="tab-pane" id="order">
                               ...
                               ';
                $i = 1;
                $all_sql = $this->all_configs['db']->query("SELECT * FROM {map} WHERE parent=?i ORDER BY `prio` ASC",
                    array($pp['id']), 'assoc:id');
                if ($all_sql) {
                    $all_sql_trans = get_few_translates(
                        'map', 'map_id',
                        $this->all_configs['db']->makeQuery("map_id IN(?q)", array(implode(',', array_keys($all_sql))))
                    );
                    foreach ($all_sql as $all_row) {
                        $all_row = translates_for_page($this->lang, $this->def_lang, $all_sql_trans[$all_row['id']],
                            $all_row, true);
                        if ($all_row['state']) {
                            $out .= '
                                <li>
            ...
            if (isset($this->all_configs['arrequest'][2]) && $this->all_configs['arrequest'][2] == 'update') {
                ...
                if (isset($_POST['sort_arr'])) {
                    ...
                    foreach ($_POST['sort_arr'] as $map_id => $sort_val) {
                        $this->all_configs['db']->query("UPDATE {map} SET prio = ?i WHERE id = ?i",
                            array($sort_val, $map_id));
                    }
                }
...
//                if ($sql) {
                ...
                if ($section != 0) {
                    $pps = $this->all_configs['db']->query("SELECT section FROM {map} WHERE id = ?i",
                        array($this->all_configs['arrequest'][1]), 'row');
                    if ($pps['section'] == $section) {
                        $sql = $this->all_configs['db']->query("UPDATE {map} SET section = ?i WHERE id = ?i",
                            array($section, $this->all_configs['arrequest'][1]));
                    } else {
                        $sql = $this->all_configs['db']->query("UPDATE {map} SET section = ?i, state=0 WHERE id = ?i",
                            array($section, $this->all_configs['arrequest'][1]));
                    }
                }//-/ if ($section!=0)
                #Сохраняем модули
...
###############################################################################
        if (isset($this->all_configs['arrequest'][1]) && $this->all_configs['arrequest'][1] == 'add') {
            ...
            if (isset($this->all_configs['arrequest'][2]) && $this->all_configs['arrequest'][2] == 'copy') {
                ...
                $copy_page = $this->all_configs['db']->query("SELECT id, url FROM {map} WHERE id = ?i",
                ...
                if (isset($this->all_configs['arrequest'][2]) && $this->all_configs['arrequest'][2]) {
                    $ppp = $this->all_configs['db']->query("SELECT * FROM {map} WHERE id = ?i",
                        array($this->all_configs['arrequest'][2]), 'row');
                    $translate = $this->all_configs['db']->query("SELECT name,lang FROM {map_strings} WHERE map_id = ?i",
                        array($ppp['id']), 'assoc:lang');
...
///////
        ...
        if (isset ($this->all_configs['arrequest'][1]) && $this->all_configs['arrequest'][1] == 'export-map-price-small-csv') {
            ...
            if ($price_table) {
                ...
                $translates0 = get_few_translates(
                    'map_prices', 'row_id',
                    $this->all_configs['db']->makeQuery("row_id IN(?q)", array(implode(',', array_keys($price_table))))
                );
...
###############################################################################
        ...
        if (isset($this->all_configs['arrequest'][1])
            ...
            $this->all_configs['db']->query("DELETE FROM {map_prices} WHERE id = ?i LIMIT 1",
                array($this->all_configs['arrequest'][2]));
            $this->all_configs['db']->query("DELETE FROM {map_prices_strings} WHERE row_id = ?i",
                array($this->all_configs['arrequest'][2]));
    ...
    private function create_page($fields, $add_module)
    {
        ...
        if ($add_module !== false) {
            if ($add_module != 'content_default') {
                $this->all_configs['db']->query("INSERT INTO {map_module}(page_id,module) VALUES(?i,?)",
                    array($id, $add_module));
            }
        }
        return $id;
    ...
    private function copy_page($source_id, $new_data)
    {
        ...
        $ppage_translates = $this->all_configs['db']->query("SELECT * FROM {map_strings} WHERE map_id = ?i",
            array($source_id), 'assoc:lang');
        ...
        foreach ($ppage_translates as $translate) {
            if (!isset($translate['lang']) || !$translate['lang']) {
                continue;
            }
            $translate['map_id'] = $id;
        ...
        $modules = $this->all_configs['db']->query("SELECT * FROM {map_module} WHERE page_id = ?i", array($source_id),
            'assoc');
        $mods = array();
    ...
    protected function ajax_choose_picture_for_page()
    {
        ...
        foreach ($files as $file) {
            ...
            if (isset($path_parts['extension']) && in_array($path_parts['extension'],
                    self::$allowed_ext) && $file != 'nopicture.jpg'
            ) {
                if ($filtr == 'small' && (strpos($file, '_m.') === false && strpos($file, '_s1.') === false)) {
                    continue;
                }

                $img_title = $this->all_configs['db']->query("SELECT name, name_en FROM {image_titles} WHERE image = ? AND gallery = ?",
                    array($file, $gallery), 'row');
                ...
                $out .= '<div class="fmfoto2">
                    ...
                    . (strpos($file,
                        '_m.') ? '<span class="crop" rel="' . $file . '">' . l('map_crop') . '</span>' : '')
        ...
        if ($crop) {
            ...
            if (isset($path_parts['extension']) && in_array($path_parts['extension'],
                ...
                $out .= '<div class="crop_foto">
                    ...
                    <div class="btn-group">'
                    . (strpos($file,
                        '_m.') ? '<button class="do_crop btn-primary btn-small btn" type="button" rel="' . $file . '">' . l('map_crop') . '</button>' : '')
                    . '</div>
                </div>';
    ...
    {
        $sql = $this->all_configs['db']->query("SELECT id FROM {image_titles} WHERE image = ? AND gallery = ?",
            array($_GET['picture'], $_GET['gallery']), 'el');
        if ($sql) {
            $upd = $this->all_configs['db']->query("UPDATE {image_titles} SET name = ?, name_en = ? WHERE `image` = ? AND `gallery` = ?",
                array($_GET['title_ru'], $_GET['title_en'], $_GET['picture'], $_GET['gallery']));
        } else {
            $ins = $this->all_configs['db']->query("INSERT INTO {image_titles}(name, name_en,image,gallery) VALUES(?,?,?,?)",
                array($_GET['title_ru'], $_GET['title_en'], $_GET['picture'], $_GET['gallery']));
        }
        ...
        if ($act == 'price_edit_row') {
            ...
            if (is_numeric($row_id) && $col) {
                if (in_array($col, array('name', 'price_copy_mark', 'price_mark', 'time_required', 'hidden'))) {
                    ...
                    $this->all_configs['db']->query("UPDATE {map_prices} SET ?col = ? WHERE id = ?i LIMIT 1",
                        array($col, $value, $row_id));
                }
                echo 'done';
            }
        }
