...
class marketing{
    ...
    private function monitoring(){
        ...
        if(isset($this->arrequest[1])){
            switch($this->arrequest[1]){
                ...
                case 'add_link_to_site':
                    ...
                    if($site && $link){
                        $this->db->query("INSERT INTO {monitoring_links}(site_id,link) VALUES(?i,?)", array($site, $link));
                    }
                    header('Location: '.$this->prefix.'marketing');
                    exit;
                break;
        ...
        foreach($sites as $site){
            $i++;
            $links_count = $this->db->query("SELECT COUNT(*) FROM {monitoring_links} WHERE site_id = ?i", array($site['id']), 'el');
