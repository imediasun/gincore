...
//            $messages = array();
            ...
            if ($id_exists) {
                ...
                $accepter = import_helper::remove_whitespace($this->provider->get_accepter($row));
                if ($accepter) {
...
//                    $errors[] = l('Не указан инженер');
                ...
                $category = import_helper::remove_whitespace($this->provider->get_category($row));
                if (!$category) {
                    ...
                    if (!$errors) {
                        try {
                            db()->query("INSERT INTO {orders} "
                                ...
                                $manager_id,
                                $device,
                                $this->accepters_wh[$accepter]['wh_id'],
                                $this->accepters_wh[$accepter]['location_id']
                            ));
                            if ($comments) {
    ...
    private function get_category_id($category)
    {
        ...
        if ($category) {
            ...
            $id = 0;
            $message = l('Не указано категория');
        }
        return array(
            'state' => $id !== false,
            'id' => $id,
