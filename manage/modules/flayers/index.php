...
class flayers
{
    ...
    private function form($flayer = array())
    {
        ...
        foreach ($categories as $category) {
            $category = translates_for_page($this->lang, $this->def_lang, $translates[$category['id']], $category,
                true);
            ...
            if ($this->all_configs['arrequest'][1] == 'sorting') {
                if (isset($this->all_configs['arrequest'][2]) && $this->all_configs['arrequest'][2] != 'save') {
                    ...
                    $translates = get_few_translates(
                        'banners',
                        'banner_id',
                        $this->all_configs['db']->makeQuery("banner_id IN (?q)",
                            array(implode(',', array_keys($flayers))))
                    );

                    $html = '';
                    foreach ($flayers as $flayer) {
                        $flayer = translates_for_page($this->lang, $this->def_lang, $translates[$flayer['id']], $flayer,
                            true);
                    ...
                    if (isset($_POST['flayer_prio'])) {
                        ...
                        foreach ($_POST['flayer_prio'] as $id => $prio) {
                            $this->all_configs['db']->query("UPDATE {banners} SET prio = ?i WHERE id = ?i",
                                array($prio, $id));
                        }
