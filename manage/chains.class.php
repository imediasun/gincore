...
class Chains extends Object
{
    ...
    function get_operations($type, $filters, $only_count = false, $goods = null)
    {
        ...
        if ($only_count == true) {
            if ($type == 1) {
                $operations = $this->all_configs['db']->query('SELECT COUNT(g.order_id)
    ...
    function show_stockman_operations($type = 1, $hash = '#orders-clients_bind')
    {
        ...
        if ($type === 1) {
            $filters = $this->view->renderFile('chains.class/binding_serial_filters', array(
                'warehouses' => db()->query('
                    SELECT id, title
                    FROM {warehouses}
                ')->assoc(),
                'locations' => isset($_GET['warehouse']) ? db()->query('
                    SELECT id, location as title
                    FROM {warehouses_locations}
                    WHERE wh_id=?i 
                ', array($_GET['warehouse']))->assoc() : array()
            ));
        } else {
    ...
    public function create_transaction($post, $mod_id = null)
    {
        ...
        try {
            ...
            if (isset($post['supplier_order_id']) && $post['supplier_order_id'] > 0) {
                ...
                $this->ContractorsCategoriesLinks->addCategoryToContractors($post['contractor_category_id_to'],
                    $post['contractors_id']);
            }
...
//            }
            ...
            if (($post['transaction_type'] == TRANSACTION_OUTPUT || $post['transaction_type'] == TRANSACTION_INPUT)) {
                ...
                if ($category_id > 0) {
                    $this->ContractorsCategoriesLinks->addCategoryToContractors($category_id,
                        intval($post['contractors_id']));
