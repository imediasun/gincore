...
class Fetcher implements \go\DB\Result
{
    ...
    /**
     * string assoc delimiter
     */
    private $assocDelimiter = ':';
    ...
    public function assoc($param = null) {
        ...
        if ($param) {
            while ($row = $imp->fetchAssoc($conn, $cursor)) {
                ...
                $params = explode($this->assocDelimiter, $param);
                if (isset($params[2])) {
                    $result[$row[$params[0]]][$row[$params[1]]][$row[$params[2]]] = $row;
                } elseif (isset($params[1])) {
                    $result[$row[$params[0]]][$row[$params[1]]] = $row;
                } else {
                    $result[$row[$params[0]]] = $row;
                }
            }
        } else {
            while ($row = $imp->fetchAssoc($conn, $cursor)) {
