<form method="post">
        ...
        <div class="row container-fluid">
            ...
            <div class="col-sm-3">
                <select class="multiselect input-small" name="cashboxes[]" multiple="multiple">
                    <?= build_array_tree($cashboxes, ((isset($_GET['cb'])) ? explode(',', $_GET['cb']) : array())) ?>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="form-group"><label><?= l('Статьи') ?>:</label>
        <div class="row container-fluid">
            <div class="col-sm-3"><select class="form-control" name="include_categories">
                    <option value="1"><?= l('Показать') ?></option>
                    <option
                        <?= ((isset($_GET['cge']) && $_GET['cge'] == -1) ? 'selected' : '') ?> value="-1">
                        <?= l('Исключить') ?>
                    </option>
                </select>
            </div>
            <div class="col-sm-3">
                <select class="multiselect form-control" name="categories[]" multiple="multiple">
                    <?= build_array_tree($categories, ((isset($_GET['cg'])) ? explode(',', $_GET['cg']) : array())); ?>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        <div class="controls">
            <input class="btn btn-primary" type="submit" name="filter-transactions" value="<?= l('Применить') ?>"/>
        </div>
    </div>

    <?php if ($isContractors == true): ?>
