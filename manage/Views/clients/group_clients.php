<form method="POST" class="form-horizontal" id="group_clients_form">
    <p>К клиенту 1 будут перенесены данные (телефоны, звонки, заказы и т.д.) клиента 2. <br>
        ...
        После этого клиент 2 будет удален.
    </p><br>
    <div class="control-group"><label class="control-label"><?= l('Клиент') ?> 1: </label>
        <div class="controls">';
            <?= typeahead($this->all_configs['db'], 'clients', false, isset($_GET['client_1']) ?
                $_GET['client_1'] : 0, 1, 'input-medium', 'input-small', '', true, false, '1') ?>
        </div>
    </div>
    <div class="control-group"><label class="control-label"><?= l('Клиент') ?> 2: </label>
        <div class="controls">';
